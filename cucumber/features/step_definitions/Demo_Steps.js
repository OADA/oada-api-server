  ...
  */
  ...
  this.Then(/^each item has at least the following information:$/, function (table,callback) {  
     for(var key in this.last_response){
         if(!check_attributes(table, this.last_response[key], function(key){   
             console.log("[PASSED] - Item Attribute Check");
         })){ callback.fail(new Error("Failed - Item Attribute Check 202")) };
         callback();
     }
  });
  ...
  this.Then(/^each item in "([^"]*)" has the following information:$/, function (this_key, table, callback) {
    ...
    for(key in root){
	    ...
	    var result = check_attributes(table, iterable, passing_msg);
	    if(!result){
		callback.fail(new Error("Failed - Iterative Attribute Check"));	
	    }
 	    fallthrough++;
    }
