...
var driver = function() {
  ...
  if (!_driver) _driver = require('./memory-db-resources-driver.js');
  return _driver;
};
...
var _RevGraph = {
  ...
  start: function() {
    ...
    .finally(function() {
      ...
      if (queue.isEmpty()) {
        _RevGraph.timer = setTimeout(_RevGraph.start, config.timeout_on_empty);
      } else {
        _RevGraph.timer = setTimeout(_RevGraph.start, config.timeout_on_notempty);
      }
    });
  },
