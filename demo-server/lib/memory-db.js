...
// use _MemoryDb.setLogger(new_logger) to override these wrappers and
// send your logs somewhere.  I'd suggest bunyan, and use a child logger
// with { module: 'memory-db'}
...
var _MemoryDb = {
  ...
  get: function(dbname, path, opts) {
    ...
    return Promise.try(function() {
      ...
      if (!_.has(db, dbname)) {
        log.error('get: Unknown database name ' + dbname);
  ...
  set: function(dbname, path, val) {
    return Promise.try(function() {
      // Check if database exists:
      if (!_.has(db, dbname)) {
        log.error('set: Unknown database name ' + dbname);
        throw new Error('set: Unknown database name ' + dbname);
      };
  ...
  remove: function(dbname, path) {
    ...
    return Promise.try(function() {
      ...
      if (!_.has(db, dbname)) {
        log.error('remove: Unknown database name ' + dbname);
        throw new Error('remove: Unknown database name ' + dbname);
      }
  ...
  // logger must have functions for error, info, debug, and trace
