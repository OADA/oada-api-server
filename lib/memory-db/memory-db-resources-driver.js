...
module.exports = function(config) {
  ...
  var rev_graph = function() {
    if (!_rev_graph) _rev_graph = config.libs.rev_graph();
    return _rev_graph;
  };
  
  // Logger:
  var log = config.libs.log().child({ module: 'memory-db-resources-driver' });
  ...
  var _MemoryDbResourcesDriver = {
    ...
    get: function(path, opts) {
      return Promise.try(function() {
        ...
        log.trace('get: returning info = ', info); 
        return info;
      });
    },
    ...
    put: function(path, topval, opts) {
      ...
      var lock_release;
      return Promise.try(function() {
        log.trace('put: path = ', path, ', topval = ', topval);
        opts = opts || {};
        if (path.charAt(0) !== '/') path = '/' + path; // leading slash is needed.
        if (path === '' || path === '/') {
