...
module.exports = function(config) {
  ...
  if (log.child) log = log.child({ module: 'memory-db' });
  ...
  var path_pointer = {
    ...
    get: function(obj, path) {
      ...
      path = path.replace(/\/+$/, ''); // trailing slashes don't work well.
      path = path.replace(/\/{2,}/g, '/'); // Ignore any double slashes
    ...
    set: function(obj, path, new_val) {
     if (!path || typeof path !== 'string') {
      ...
      log.trace('path_pointer.set: setting at path ', path, ', new_val = ', new_val);
      path = path.replace(/\/+$/, ''); // trailing slashes don't work well.
      path = path.replace(/\/{2,}/g, '/'); // Ignore any double slashes
      if (!path.match(/^\//)) path = '/' + path; // json-pointer likes leading slashes
      if (path.match(/^\/$/)) path = ''; // json-pointer doesn't like ONLY slashes
      ...
      if (!key_to_set) {
        log.error('path_pointer.set: could not find key to set: ', key_to_set);
        return false;
      }
      if (!parent_to_change) {
  ...
  var _MemoryDb = {
    ...
    set: function(dbname, path, val) {
      return Promise.try(function() {
        ...
        log.trace('set: path = ', path, ', val = ', val);
    ...
    clean: function(dbname) {
      return Promise.try(function() {
        if (!dbname) {
          db = {}; // clean all of them
          return true;
        }
        db[dbname] = {}; // clean a specific one of them
        return true;
      });
    },
